...
export function useContractOperations({
  ...
  const validateContract = useCallback((contractData: Partial<Contract>): string[] => {
    const errors: string[] = [];
    ...
    if (contractData.guaranteedMoney && contractData.totalValue && 
    ...
    if (league && league.salaryCap && contractData.currentSalary && contractData.currentSalary > league.salaryCap * 0.3) {
  ...
  // Criar novo contrato
  const createContract = useCallback(async (player: Player, contractData: Partial<Contract>): Promise<ContractOperationResult> => {
    setIsLoading(true);
    setError(null);
  ...
  const updateContract = useCallback(async (contract: Contract, updates: Partial<Contract>): Promise<ContractOperationResult> => {
    setIsLoading(true);
    setError(null);
  ...
  // Estender contrato
  const extendContract = useCallback(async (
    contract: Contract, 
    ...
    try {
      ...
      // Disparar evento de atualização
      window.dispatchEvent(new CustomEvent('contractExtended', { 
  ...
  const applyFranchiseTag = useCallback(async (player: Player, tagValue: number): Promise<ContractOperationResult> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      // Disparar evento de atualização
      window.dispatchEvent(new CustomEvent('franchiseTagApplied', { 
  ...
  // Cortar jogador
  const cutPlayer = useCallback(async (player: Player, contract: Contract): Promise<ContractOperationResult> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      // Disparar evento de atualização
      window.dispatchEvent(new CustomEvent('playerCut', { 
