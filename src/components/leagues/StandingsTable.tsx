...
interface StandingsTableProps {
  ...
  /** Mensagem de erro */
  error?: string | null;
  /** Número de times que se classificam para playoffs */
...
export function StandingsTable({
  ...
  // Mostrar erro apenas se não há dados E há erro
  if (standings.length === 0 && error) {
    return (
      <div className="text-center py-12">
        <div className="text-4xl mb-4">⚠️</div>
        <h3 className="text-lg font-medium text-slate-100 mb-2">Erro ao carregar classificação</h3>
        <p className="text-slate-400">{error}</p>
      </div>
    );
  }

  // Mostrar mensagem de dados vazios apenas se não há dados E não há erro
  if (standings.length === 0 && !error) {
    return (
      <div className="text-center py-12">
        <div className="text-4xl mb-4">📊</div>
        <h3 className="text-lg font-medium text-slate-100 mb-2">Nenhum time encontrado</h3>
        <p className="text-slate-400">Não há times cadastrados nesta liga.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-0">
      {/* Aviso de erro quando há dados */}
      {error && standings.length > 0 && (
        <div className="bg-yellow-900/20 border border-yellow-700 rounded-t-2xl p-3">
          <div className="flex items-center gap-2 text-yellow-400">
            <span className="text-sm">⚠️</span>
            <span className="text-sm">
              Dados do Sleeper indisponíveis. Exibindo dados locais (pode estar na offseason).
            </span>
          </div>
        </div>
      )}
      ...
      <div
        className={`bg-slate-800 border border-slate-700 ${error && standings.length > 0 ? 'border-t-0' : 'rounded-t-2xl'} border-b-0 p-4`}
      >
      ...
      <div className="bg-slate-800 border border-slate-700 border-t-0 rounded-b-2xl overflow-hidden">
              ...
              {filteredStandings.map(standing => {
                return (
                  ...
                  >
                    ...
                    {/* Streak */}
                    <td className="px-6 py-4 whitespace-nowrap">{renderStreak(standing.streak)}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm">
                        ...
                        <div className="text-xs text-slate-400">
                          {league
                            ? (
                                (standing.financialSummary.totalSalaries / league.salaryCap) *
                                100
                              ).toFixed(1)
                            : '0'}
                          % usado
        ...
        <div className="p-4 bg-slate-700/50 border-t border-slate-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 text-xs text-slate-400">
            <div>
              <strong>Pos:</strong> Posição na classificação
            </div>
            <div>
              <strong>V/D/E:</strong> Vitórias/Derrotas/Empates
            </div>
            <div>
              <strong>PF/PA:</strong> Pontos Feitos/Pontos Contra
            </div>
            <div>
              <strong>Streak:</strong> Sequência atual (W=Vitória, L=Derrota)
            </div>
            <div>
              <strong>Cap Disponível:</strong> Salary cap restante
            </div>
            <div>
