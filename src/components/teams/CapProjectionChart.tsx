...
export function CapProjectionChart({
  ...
  const CustomTooltip = ({ active, payload, label }: TooltipProps) => {
    if (active && payload && payload.length) {
      ...
      const usagePercentage = calculateCapUsagePercentage(
        data.totalSalaries + data.deadMoney,
        salaryCap,
      );
            ...
            <div className="border-t border-slate-700 pt-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-slate-100 font-bold font-mono">
                  {formatCurrency(data.totalSalaries + data.deadMoney)}
                </span>
              </div>
            </div>
  ...
  return (
    <div className="bg-slate-800 rounded-xl shadow-xl border border-slate-700 p-4 sm:p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-xs text-slate-400 hidden sm:block">Próximas 4 temporadas</div>
      </div>
              ...
              {projectionData.map((data, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className="py-3 px-4 text-center text-slate-300 text-sm hidden sm:table-cell"
                      aria-label={`${data.contractsCount} contratos ativos`}
                    >
                      {data.contractsCount}
                    </td>
                    <td
                      className="py-3 px-4 text-right text-slate-300 font-mono text-sm"
                      aria-label={`Salários: ${formatCurrency(data.totalSalaries)}`}
                    >
                      {formatCurrency(data.totalSalaries)}
                    </td>
                    <td
                      className="py-3 px-4 text-right font-mono text-sm hidden md:table-cell"
                      aria-label={
                        hasDeadMoney
                          ? `Dead money: ${formatCurrency(data.deadMoney)}`
                          : 'Sem dead money'
                      }
                    >
                      {hasDeadMoney ? (
                    ...
                    <td
                      className="py-3 px-4 text-right font-medium font-mono text-slate-100 text-sm"
                      aria-label={`Total comprometido: ${formatCurrency(data.totalCommitted)}`}
                    >
                      {formatCurrency(data.totalCommitted)}
                    </td>
                    <td
                      className={`py-3 px-4 text-right font-medium font-mono text-sm ${
                        data.availableCap >= 0 ? 'text-green-400' : 'text-red-400'
                      }`}
                    ...
                    <td
                      ...
                      aria-label={`Uso do cap: ${formatPercentage(usagePercentage)}`}
                    >
                        ...
                        <div className="flex space-x-1">
                          {isOverCap && (
                            <span
                              ...
                              title="Acima do salary cap"
                              aria-label="Acima do salary cap"
                            >
                              !
                            </span>
                          )}
                          {!isOverCap && isCritical && (
                            <span
                              ...
                              title="Uso crítico do salary cap"
                              aria-label="Uso crítico do salary cap"
                            >
                              ⚠
                            </span>
                          )}
                          {hasDeadMoney && data.deadMoney > 5000000 && (
                            <span
                              className="inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-purple-500 rounded-full"
                              title="Alto dead money"
                              aria-label="Alto dead money"
                            >
                              D
                            </span>
                          )}
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="text-sm text-red-300 mt-1">
                  O time está projetado para exceder o salary cap em{' '}
                  {projectionData.filter(d => d.availableCap < 0).length} temporada(s). Considere
                  cortar jogadores ou renegociar contratos.
                </p>
                <div className="mt-2 text-xs text-red-400">
                  Anos afetados:{' '}
                  {projectionData
                    .filter(d => d.availableCap < 0)
                    .map(d => d.year)
                    .join(', ')}
                </div>
              </div>
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="text-sm text-orange-300 mt-1">
                  O time está usando mais de 95% do salary cap em algumas temporadas. Pouco espaço
                  para movimentações.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="text-sm text-purple-300 mt-1">
                  O time tem dead money significativo ({'>'}$10M) em algumas temporadas, impactando
              ...
              <div className="flex items-start">
                ...
                <div className="ml-3 flex-1">
                  ...
                  <p className="text-sm text-green-300 mt-1">
                    O time está em boa situação financeira com espaço adequado no salary cap para as
                    próximas temporadas.
                  </p>
                </div>
              </div>
